...
class RailwayAuditTriggerHandler(_BaseDiagnosticHandler):
    def do_GET(self):
        parsed = urlparse(self.path)
        if parsed.path == "/internal/coreapp_audit_trigger":
            try:
                result = live81._coreapp_audit_result()
                print("COREAPP_AUDIT_TRIGGER " + json.dumps(result, ensure_ascii=False, sort_keys=True), flush=True)
                return self._send_json(200, {"ok": True})
            except Exception as exc:
                print("CoreApp Railway audit error:", type(exc).__name__, flush=True)
                return self._send_json(500, {"ok": False})
        return super().do_GET()


bot.CoreAppWebhookHandler = RailwayAuditTriggerHandler
...
if __name__ == "__main__":
    ...
    print("Railway-triggered CoreApp production audit ready", flush=True)
